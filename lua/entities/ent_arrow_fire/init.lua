	...
	if ent:IsNPC() or ent:IsPlayer() or ent:IsWorld() or ent:IsVehicle() or ent:IsValid() then
		...
		if speed < 150 then
			...
			if speed >= 150 then
                ...
                ent:Ignite(5) 
                ent:SetNWFloat("MM_FireDuration", CurTime() + 5)
                ent:SetNWInt("MM_FireDamage", 3)
                ent:SetNWEntity("MM_FireOwner", self.Owner)
                ent:SetNWEntity("MM_FireInflictor", self.Inflictor)			
				self:Remove()
			end
			return false end
			if ent:IsWorld() then
				...
				if ent:IsValid() && ent:GetNWFloat("DivingRight") < CurTime() && ent:GetNWFloat("DivingLeft") < CurTime() then
                    ...
                    ent:SetNWInt("MM_FireDamage", 3)
                    ent:SetNWEntity("MM_FireOwner", self.Owner)
                    ent:SetNWEntity("MM_FireInflictor", self)					
