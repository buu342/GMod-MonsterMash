...
function SWEP:Deploy()
	...
	timer.Simple(0.5,function() if !IsValid(self) then return end self:SetPrimed(true) end)
...
function SWEP:Think()
	...
	if self.Weapon:Clip1() < 1 then
		...
		return
	elseif self.Owner:GetNWFloat("mm_web_recharge") != 0 then
		self.Owner:GetViewModel():SetPlaybackRate( 1 )
		self.Weapon:SendWeaponAnim( ACT_VM_DRAW )
		self:SetNextPrimaryFire(CurTime()+0.5)
		self.Owner:SetNWFloat("mm_web_recharge",0)
        self:SetPrimed(true)
	end

	if self:CanPrimaryAttack() && self.Owner:KeyPressed(IN_ATTACK) && self:GetPrimed() == true then
        self:SetPrimed(false)
        ...
        if SERVER then
            ...
            local grenade = ents.Create("ent_spidernade")
            ...
            grenade:Spawn()
            grenade:SetNWEntity("Sticky_Inflictor",self)
            grenade:SetNWEntity("Sticky_Attacker",self.Owner)
            grenade:Activate()
		...
		self.Owner:SetNWFloat("mm_web_recharge", CurTime()+20)
