SWEP.SelectIcon = "vgui/entities/mm_urn"
SWEP.Cost = 25
...
function SWEP:Initialize()
	...
	self.Owner:SetNWFloat("mm_urn_recharge", 0)
end
...
function SWEP:Think()
	...
	if self.Weapon:Clip1() < 1 then
		...
		return
	elseif self.Owner:GetNWFloat("mm_urn_recharge") != 0 then
	...
	if !self.Owner:KeyDown(IN_ATTACK) && self:GetPrimed() == true && self:GetMM_PrimeTime() > 175 then
		...
		if SERVER then
			...
			local grenade = ents.Create("ent_urn")
			...
			grenade:Spawn()
			grenade:Activate()
            grenade:SetNWEntity("MM_HauntedUrnInflictor", self)
		...
		self.Owner:SetNWFloat("mm_urn_recharge", CurTime()+20)
...
hook.Add("CalcMainActivity", "animations", function(ply, velocity)
    if IsValid(ply) && ply:GetNWFloat("Spooked") > CurTime() then
