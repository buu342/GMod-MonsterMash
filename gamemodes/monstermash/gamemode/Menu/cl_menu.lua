local version = "Version 2.00 Release" 
...
local story = {
    ...
    "You were hibernating in your coffin when one day you heard some commotion coming from the ground above and disturbing your beauty sleep. Aggitated, you climb out of your grave to see what the fuss is about. It's time to dust off the old guns, shut this party down, and get back to resting in peace!",
...
local updates = {
    ...
    "",
    "Rewrote the gamemode from scratch!",
    "Added more wacky rounds"
}
...
local adminoptions = {
    ...
    [04] = { text = "Point Limit",                 command = "mm_point_limit" },
    [05] = { text = "Rounds to Map Swap",          command = "mm_maxrounds" },
    [06] = { text = "Wacky Round Frequency",       command = "mm_wackyfrequency" },
    [07] = " ",
    [08] = { text = "Gold Limit",                  command = "mm_budget" },
    [09] = { text = "Cleanup Time",                command = "mm_cleanup_time" },
    [10] = { text = "Health Regen Time",           command = "mm_healthregen_time" },
    [11] = { text = "Health Regen Amount",         command = "mm_healthregen_amount" },
    [12] = " ",                                     
    [13] = { text = "Spawn Protection",            command = "mm_spawnprotect", checkbox = true },
    [14] = { text = "Ludicrous Gibs Mode",         command = "mm_ludicrousgibs", checkbox = true },
...
function DrawWelcome(frame, w, h)
    ...
    text:SetText(story[math.random(#story-1)+1].."\n\n"..welcomemessage)
    text:SetAutoStretchVertical(true)
    text:SetWrap(true)
    local garb, finaly = surface.GetTextSize(story[math.random(#story-1)+1].."\n\n"..welcomemessage)
    finaly = finaly + h/7
...
function UpdateLoadoutText(frame, info, w, h)
    ...
    if (finaldesc != "") then
        ...
        if (info.Weptable.points != nil) then
...
function DrawLoadout(frame, w, h)
    ...
    for i=1, MaxWeapons do
        ...
        Weapons[i].Panel.Paint = function(self, w, h)
            ...
            if (LocalPlayer():GetWeaponTable() != nil && ((Weapons[i].Category != "Trick" && LocalPlayer():GetWeaponTable()[Weapons[i].Category] == Weapons[i].Class) || (LocalPlayer():GetWeaponTable()["Trick_NextLife"] == Weapons[i].Class))) then
                surface.SetDrawColor(Color(81, 47, 112, 255))
                surface.DrawOutlinedRect(0, 0, w, h, 5)
            end
        end
    ...
    for i=1, #adminoptions do
        ...
        if (v != " ") then
            if (v.checkbox) then
                ...
                TextEntry:SetPos( x+168*wratio, y )
