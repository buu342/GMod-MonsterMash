...
function GM:DoPlayerDeath(victim, attacker, dmginfo)
    ...
    if (dmginfo:GetDamageType() != DMG_DISSOLVE) then
        ...
        elseif (victim:HasStatusEffect(STATUS_ONFIRE)) then
            ...
            if victim:IsOnGround() && math.random(1, 3) == 1 && !victim:MissingBothLegs() then
                self:GoreScripted(victim, inflictor)
            else
...
function SpectateEntity(ply, ent)
    if !IsValid(ply) || ply == NULL then return end
    if !IsValid(ent) || ent == NULL then return end
    ply:SpectateEntity(ent)
    ply:Spectate(OBS_MODE_CHASE)
    timer.Create(tostring(ent).."_spectate", GetConVar( "mm_cleanup_time" ):GetInt(), 0, function()
        if !IsValid(ply) || ent == NULL then return end
        if !IsValid(ent) || ent == NULL then 
            if timer.Exists( tostring(ent).."_spectate" ) then
                timer.Remove( tostring(ent).."_spectate" )
            end
            return 
        end
        
        if (ply:GetObserverTarget() != ent && (IsValid(ent) || ent == NULL)) then
            ent:Remove()
            if timer.Exists( tostring(ent).."_spectate" ) then
                timer.Remove( tostring(ent).."_spectate" )
            end
...
function GM:GoreScripted(ply, wep)
    ...
    elseif wep:GetClass() == "weapon_mm_stake" then
        ...
        if math.random(1, 2) == 1 && ply.deathang != "" && ply.deathang != "Back" then
...
function GM:GoreMelter(ply)
    ...
    ent:EmitSound("weapons/melter/melted.wav", 128)
...
function GM:GoreDecapitate(ply)
    ...
    local head = self:GoreCreateGib(ply, ply:GetCharacter().gib_head, ply:GetPos()+Vector(0, 0, 65), ply:GetAngles(), 1, nil, false)
...
function GM:GoreHeadExplode(ply)
    ...
    self:GoreCreateGib(ply, ply:GetCharacter().gib_headbits, ply:GetPos()+Vector(0,0,2), ply:GetAngles(), 0, true)
    SpectateEntity(ply, ent)
end
...
function GM:GoreCreateGib(ply, model, pos, ang, overrideforce, ragdoll, remove)
    ...
    if (remove || remove == nil) then
        timer.Simple(GetConVar("mm_cleanup_time"):GetInt(), function() 
            if !IsValid(ent) then return end 
            ent:Remove() 
        end)
    end
...
function GM:GoreCreateRagdoll(ply, model, pos, ang, matchbones, extra, remove, wep)
    ...
    ent:Spawn()
    ent:SetCollisionGroup(COLLISION_GROUP_WEAPON)
    if wep != nil && wep:GetClass() == "weapon_mm_stake" then
        ent:SetBodygroup(GIBGROUP_STAKE, GIBGROUP_STAKE_ON)
    end
    if extra == nil then
        ent:SetBodygroup(GIBGROUP_ARMS, ply:GetBodygroup(GIBGROUP_ARMS))
