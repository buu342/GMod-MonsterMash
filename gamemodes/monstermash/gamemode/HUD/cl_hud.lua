...
hook.Add("HUDPaint", "MM_HUD", function()
        ...
        if (GAMEMODE:GetRoundState() == GMSTATE_BUYTIME) then
            if (GAMEMODE:InWackyRound()) then
                ...
                if (LocalPlayer() == GAMEMODE:GetSuperPlayer()) then
                    ...
                    draw.SimpleTextOutlined( GAMEMODE:WackyRoundData().round_message1, "MMDefaultFont", ScrW()/2, ScrH()/2+ysize, Color(255,105,0,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0,0,0,255))
                end
            else
                local validplayers = {}
    ...
    if LocalPlayer():Alive() && LocalPlayer():Team() != TEAM_COOPDEAD && LocalPlayer():Team() != TEAM_SPECT then
        ...
        if !LocalPlayer():MissingBothLegs() then
            ...
            if (LocalPlayer():GetAbilityCooldown() > 0) then
                stamina = staminasize*(1-(LocalPlayer():GetAbilityCooldown())/5)
            end
            surface.SetDrawColor( 255, 115, 0, 255 ) 
    ...
    *****************************************/
    if LocalPlayer():Alive() && LocalPlayer():Team() != TEAM_COOPDEAD && LocalPlayer():Team() != TEAM_SPECT then
        ...
        if (LocalPlayer():Team() != TEAM_COOPMONST) then
            ...
            if (LocalPlayer():GetWeaponTable() != nil) then
                ...
                if (LocalPlayer():HasStatusEffect(STATUS_INVISIBLE)) then
                    trickamount = LocalPlayer():GetStatusEffectTime(STATUS_INVISIBLE)/10
        ...
        // Trick icon
        if (LocalPlayer():Team() != TEAM_COOPMONST && LocalPlayer():GetWeaponTable() != nil && GAMEMODE.Weapons["Trick"][LocalPlayer():GetWeaponTable()["Trick"]].icon != -1) then
        ...
        if #scorelist > 0 then
            for k, v in pairs(scorelist) do
                ...
                draw.SimpleTextOutlined("+"..tostring(v.score), "MMDefaultFont", ScrW()-56*hratio, ScrH()-(76-(1-time)*100)*hratio, Color(0, 255, 0, 255*time), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, 255*time))
    ...
    *****************************************/
    ...
    /*****************************************
                Press E to pick up
    *****************************************/
    if LocalPlayer():Alive() && tr.Hit && tr.Entity:IsWeapon() && LocalPlayer():Team() != TEAM_COOPDEAD && LocalPlayer():Team() != TEAM_SPECT then
        ...
        if hinttime < CurTime() then
            ...
            if (IsValid(LocalPlayer():GetNWEntity("MM_Killer"))) then
                if (LocalPlayer():GetObserverMode() == OBS_MODE_FREEZECAM) then
                    ...
                    elseif LocalPlayer():GetNWEntity("MM_Killer"):GetClass() == "sent_mm_skellington" then
            ...
            if (GetConVar("mm_deathtips"):GetInt() == 1) then
                ...
                local text_w, text_h = select( 1, surface.GetTextSize( text[1] ))
