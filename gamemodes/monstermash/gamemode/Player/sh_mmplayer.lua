...
function metaplayer:SetLastDamage(dmginfo)
    ...
    if (dmginfo:GetAttacker() != self) then
        self:SetNWInt("MM_PlayerLastDamageAttacker", dmginfo:GetAttacker())
        self:SetNWInt("MM_PlayerLastDamageInflictor", dmginfo:GetInflictor())
    end
    ...
    self.dmgtable[tostring(dmginfo:GetAttacker())].attacker = dmginfo:GetAttacker()
    self.dmgtable[tostring(dmginfo:GetAttacker())].inflictor = dmginfo:GetInflictor()
...
function metaplayer:ResetAbilityCoolDown(time)
    ...
    self:SetNWFloat("MM_PlayerAbilityCoolDown", CurTime() + (time or 5))
end
...
function metaplayer:GetLastDamage()
    local dmg = {
        ...
        attacker = self:GetNWInt("MM_PlayerLastDamageAttacker"),
        inflictor = self:GetNWInt("MM_PlayerLastDamageInflictor"),
...
function metaplayer:SetStatusEffect( effect, dmginfo, duration )
    if (self:IsDodgeRolling()) then return end
...
function metaplayer:SetWeaponCooldown(weapon, time)
    self:SetNWFloat("MM_WeaponCooldown_"..weapon:GetClass(), CurTime() + time)
    self:SetNWInt("MM_WeaponCooldownMax_"..weapon:GetClass(), time)
    if self.weaponCooldowns == nil then
        self.weaponCooldowns = {}
    end
    table.insert(self.weaponCooldowns, weapon:GetClass())
    if SERVER then
...
function metaplayer:ResetAllWeaponCooldowns()
    ...
    for i=1, #self.weaponCooldowns do
        self:SetNWFloat("MM_WeaponCooldown_"..self.weaponCooldowns[i], 0)
    end
end
...
function metaplayer:ResetLifeKills()
...
function metaplayer:ActivateTrick()
    ...
    if (self:Team() == TEAM_COOPMONST) then return end
    self:ResetTreatStack()
