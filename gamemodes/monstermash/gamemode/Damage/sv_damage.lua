...
function GM:EntityTakeDamage( victim, dmginfo )
    ...
    if (GAMEMODE:GetSuperPlayer() == victim && (dmginfo:GetInflictor() == "weapon_mm_stake" || dmginfo:GetDamage() == 9001)) then
        ...
        while (limb != 0) do
            ...
            if limb == 1 then
                if !victim:MissingLeftArm() then
                    ...
                    self:GoreCreateGib(victim, victim:GetCharacter().gib_armleft, victim:GetPos(), victim:GetAngles(), 1)
            ...
            if limb == 2 then
                if !victim:MissingRightArm() then
                    ...
                    victim:EmitSound("ambient/machines/slicer1.wav")
                    self:GoreCreateGib(victim, victim:GetCharacter().gib_armright, victim:GetPos(), victim:GetAngles(), 1)
                    if victim:MissingLeftArm() then
                        victim:SetBodygroup(GIBGROUP_ARMS, GIBGROUP_ARMS_NONE)
                    else
            ...
            if limb == 3 then
                if !victim:MissingLeftLeg() then
                    ...
                    victim:EmitSound("ambient/machines/slicer1.wav")
                    self:GoreCreateGib(victim, victim:GetCharacter().gib_legleft, victim:GetPos(), victim:GetAngles(), 1)
            ...
            if limb == 4 then
                if !victim:MissingRightLeg() then
                    ...
                    victim:EmitSound("ambient/machines/slicer1.wav")
                    self:GoreCreateGib(victim, victim:GetCharacter().gib_legright, victim:GetPos(), victim:GetAngles(), 1)
                    if victim:MissingLeftLeg() then
                        victim:SetBodygroup(GIBGROUP_LEGS, GIBGROUP_LEGS_NONE)
                    else
