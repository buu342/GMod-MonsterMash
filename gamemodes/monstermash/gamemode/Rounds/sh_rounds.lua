...
function GM:BeginRound()
    ...
    if (self:InWackyRound() && self.WackyRounds[GAMEMODE:GetWackyRound()].limit_time != nil) then
        time = self.WackyRounds[GAMEMODE:GetWackyRound()].limit_time
    end
...
function GM:UpdateRound()   
    ...
    if (self:GetRoundState() == GMSTATE_ROUND) then
        ...
        if (!self:InWackyRound() || GAMEMODE.WackyRounds[self:GetWackyRound()].mode == MODE_DEATHMATCH) then
            ...
            if (GAMEMODE.WackyRounds[self:GetWackyRound()].mode == MODE_LMS) then
                local lastcount = {}
                for k, v in pairs(player.GetAll()) do
            ...
            elseif (GAMEMODE.WackyRounds[self:GetWackyRound()].mode == MODE_CONVERT) then
                local lastcount = {}
                for k, v in pairs(player.GetAll()) do
                ...
                if (#lastcount == 0) then
                    self:SetRoundWinnerType(TEAM_INVALID)
                    self:SetRoundWinnerName("The skeletons have spooked the whole land")
                    self:EndCurrentRound()
                elseif self:GetRoundTime() < CurTime() then
                    self:SetRoundWinnerType(TEAM_INVALID)
                    self:SetRoundWinnerName("The monsters were not spooked")
                    self:EndCurrentRound()
                end
            end
        end
    ...
    if (self:GetRoundState() == GMSTATE_ENDING) then
        ...
        if SERVER then
            if (self:GetRoundEndTime() < CurTime()+3 && game.GetTimeScale() != 1) then
                game.SetTimeScale(1)
            end
        end
        ...
        if (self:GetRoundEndTime() < CurTime() && SERVER) then
            ...
            if (self:GetRoundsToWacky() <= 0 && #validplayers > 1) then
                local _, round = table.Random(GAMEMODE.WackyRounds)
                if ((!self:InWackyRound() && (GAMEMODE.WackyRounds[round].mode == MODE_PVSUPER || GAMEMODE.WackyRounds[round].mode == MODE_CONVERT)) || (self:InWackyRound() && self:WackyRoundData().mode == MODE_PVSUPER || self:WackyRoundData().mode == MODE_CONVERT)) then
                    self:SetSuperPlayer(table.Random(validplayers))
                end
                if (!self:InWackyRound()) then
                    self:SetWackyRound(round)
                end
                ...
                if (self:GetRoundsToWacky() <= 0 && #validplayers < 2) then
                    self:SetRoundsToWacky(GetConVar("mm_wackyfrequency"):GetInt())
                end
            end
            self:StartNewRound()
