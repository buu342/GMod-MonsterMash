...
function ENT:Think()
    ...
    for k, v in pairs(player.GetAll()) do
        if v != self.Owner && v:GetPos():Distance(self:GetPos()) < self:GetModelScale()*90 && v:GetPos().z > self:GetPos().z-5 && v:GetPos().z < self:GetPos().z+10 && self:VisibleVec( v:GetPos()+Vector(0,0,50)) && !v:IsDodgeRolling() then
